...
public class YRule extends TransliterationRule {
    ...
    public RuleResult apply(char[] chars, int i) {
        ...
        if (current == 'ե' && Utils.isStartOfWord(chars, i)) {
            if (i + 1 < chars.length && Character.toLowerCase(chars[i + 1]) == 'ն' && Utils.isEndOfWord(chars, i + 1)) {
                return new RuleResult(formatOutput("en", isUpperCase), 1);
            }
            if (i + 2 < chars.length && Character.toLowerCase(chars[i + 1]) == 'ն' && Character.toLowerCase(chars[i + 2]) == 'ք' && Utils.isEndOfWord(chars, i + 2)) {
                return new RuleResult(formatOutput("enk", isUpperCase), 2);
            }
            if (i + 1 < chars.length && Character.toLowerCase(chars[i + 1]) == 'մ' && Utils.isEndOfWord(chars, i + 1)) {
                return new RuleResult(formatOutput("em", isUpperCase), 1);
            }
            if (i + 1 < chars.length && Character.toLowerCase(chars[i + 1]) == 'ս' && Utils.isEndOfWord(chars, i + 1)) {
